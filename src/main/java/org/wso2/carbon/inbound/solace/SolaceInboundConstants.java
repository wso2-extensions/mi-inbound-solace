...
public final class SolaceInboundConstants {
    ...
    /**
     * Non-null placeholder for {@link com.solacesystems.jcsmp.JCSMPProperties#USERNAME}. JCSMP
     * requires a non-null username to build the SMF login frame even when authentication is by
     * client certificate or OAuth2, where the broker derives the client identity from the cert /
     * token rather than this field (under the default broker configuration). Named so it does not
     * read like an unfilled template value.
     */
    public static final String PLACEHOLDER_USERNAME = "wso2-mi-inbound";
